...
def init_upload(event, context):
    ...
    try:
        ...
        response = s3_client.generate_presigned_url(
            "put_object", Params=params, ExpiresIn=360
        )
        return {
            "statusCode": 200,
            "headers": {
            ...
            "body": json.dumps({"presignedUrl": response}),
        }
...
def delete_with_prefix(prefix: str):
    ...
    list_of_uploaded_files = s3_client.list_objects_v2(
        Bucket=content_bucket, Prefix=file_key_prefix + prefix, MaxKeys=1000
    )
    ...
    for key in list(map(lambda x: x.get("Key"), all_files)):
        s3_client.delete_object(Bucket=content_bucket, Key=key)
