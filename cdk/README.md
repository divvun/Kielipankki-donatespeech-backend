...
Create the needed hosted zone e.g. example.com and direct the domain to that DNS
server.
...
When the deployment is done, WAF needs to be attached to the CloudFront
distribution manually ATM. So do this now.

After the production account is created, create the hosted zone for the dev
environment and then create the NS record to production to point to the dev env
hosted zone.
